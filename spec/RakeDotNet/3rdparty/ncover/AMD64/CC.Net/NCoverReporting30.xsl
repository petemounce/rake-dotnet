<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!--
			...
			NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE
			-->
	<xsl:variable name="baseimagepath" xml:space="preserve">
		<xsl:choose>
	...
	<xsl:variable name="reportName" xml:space="preserve">
		<xsl:choose>
			<xsl:when test="string($reportnameoverride) != ''">
				<xsl:value-of select="$reportnameoverride"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="string(//./params[@name = 'reportname']/@value) != ''">
						<xsl:value-of select="//./params[@name = 'reportname']/@value"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$execution/projectname"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:variable name="diffAtLeast" xml:space="preserve">
		<xsl:choose>
			<xsl:when
					test="string(//./params[@name = 'diffatleast']/@value) != '' and string(number(//./params[@name = 'diffatleast']/@value)) != 'NaN'">
				<xsl:value-of select="number(//./params[@name = 'diffatleast']/@value)"/>
			</xsl:when>
	...
	<xsl:variable name="vcColumnCount" xml:space="preserve">
		<xsl:choose>
			<xsl:when test="$reportVisitCounts">2</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="ccClassCount" xml:space="preserve">
		<xsl:choose>
	...
	<xsl:variable name="ccColumnCount" xml:space="preserve">
		<xsl:choose>
			<xsl:when test="$reportCC">1</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="tablecolumns" xml:space="preserve">
		<xsl:choose>
			<xsl:when test="$isDiffReport">16</xsl:when>
			<xsl:when test="$isTrendReport">6</xsl:when>
			<xsl:when test="not ($isFunctionReport) and $reportBranchPoints">
				<xsl:value-of select="9 + $ccColumnCount"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="5 + $ccColumnCount + $vcColumnCount"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="reportTitle">
		<xsl:choose xml:space="preserve">
			...
			<xsl:when test="$reportType = 'MethodModuleNamespaceClassMethod'">Method Module, Namespace, Class and Methods
			</xsl:when>
			...
			<xsl:when test="$reportType = 'SymbolModuleNamespaceClassMethod'">Symbol Module, Namespace, Class and Methods
			</xsl:when>
			...
			<xsl:when test="$reportType = 'SymbolCCModuleClassFailedCoverageTop'">
				Cyclomatic Complexity Coverage Top
				<xsl:value-of select="$ccClassCount"/>
				Failing Coverage
			</xsl:when>
			...
			<xsl:when test="$reportType = 'MethodCCModuleClassFailedCoverageTop'">
				Cyclomatic Complexity Function Coverage Top
				<xsl:value-of select="$ccClassCount"/>
				Failing Coverage
			</xsl:when>
				...
				<html>
					<head>
						<title>
							...
							-
							<xsl:value-of select="$filterStyle"/>
							-
							<xsl:value-of select="$sortStyle"/>
						</title>
						<xsl:comment>NCover 3.0 Template</xsl:comment>
						<xsl:comment>Generated by the Joe Feser joe@ncover.com on the NCover team and inspired by the original
							template by Grant Drake
						</xsl:comment>
	...
	<xsl:template match="trendcoveragedata">
		...
		<xsl:if test="$reportType = 'SymbolSourceCodeClass'
			...
			<xsl:call-template name="documentSummaryCommon">
				...
				<xsl:with-param name="printMethods">
					<xsl:choose xml:space="preserve">
						<xsl:when test="$reportType = 'SymbolSourceCodeClassMethod' or $reportType = 'MethodSourceCodeClassMethod'">
							True
						</xsl:when>
		...
		<xsl:if
				test="($reportType = 'SymbolModuleNamespaceClassMethod') or ($reportType = 'MethodModuleNamespaceClassMethod')">
			<xsl:call-template name="summaryCommon">
				<xsl:with-param name="threshold" select="$threshold"/>
				<xsl:with-param name="unvisitedTitle" select="$unvisitedTitle"/>
				<xsl:with-param name="coverageTitle" select="$coverageTitle"/>
				...
				<table width="100%">
					<tbody>
						<tr>
							<td valign="top">
								<h1 class="titleText">
									NCover Report -<xsl:value-of select="$reportName"/>&#160;-&#160;
									<xsl:value-of select="$reportTitle"/>
								</h1>
									...
									<tbody>
										<xsl:if test="string(//./params[@name = 'coveragedate']/@value) != ''">
											<tr>
												...
												<td>
													<xsl:value-of select="string(//./params[@name = 'coveragedate']/@value)"/>&#160;at&#160;
													<xsl:value-of select="string(//./params[@name = 'coveragetime']/@value)"/>
												</td>
											</tr>
										</xsl:if>
										<tr>
									...
									<tbody>
										<tr>
											<td valign="top" nowrap="true" class="heading">
												<xsl:attribute name="rowspan" xml:space="preserve">
													<xsl:choose>
														<xsl:when test="$reportBranchPoints">5</xsl:when>
										...
										<xsl:if test="count($root/doc[@id != 0]) = 0 or $projectstats/@usp + $projectstats/@vsp = 0">
											<tr>
												<td colspan="6" align="right" class="heading">
													Symbols were not available for this execution.<br/>The sequence points coverage data will not
													be available.
												</td>
											</tr>
										</xsl:if>
										<xsl:if
												test="(not ($trendsdataexist) and $isTrendReport) or (string($projectstats/@dvsp) = '' and $isDiffReport)">
											<tr>
												<td colspan="6" align="right" class="heading">
						...
						<xsl:if test="name(.) != 'trendcoveragedata' or not ($root)">
							<tr>
								<td>
									<b>
										The input file used to generate the report must start with 'trendcoveragedata' and the file provided
										started with '<xsl:value-of select="name(.)"/>'
									</b>
								</td>
							</tr>
						</xsl:if>
						<xsl:if test="name(.) = 'coverage'">
							<tr>
								<td>
									<b>
										The input file used to generate the report is a coverage file and not a report file. Please use
										ncover.reporting to generate an xml file format that may be used with this xsl file.
									</b>
								</td>
							</tr>
						</xsl:if>
	...
	<xsl:template name="sectionTableHeader">
		...
		<xsl:variable xml:space="preserve" name="qualifiedmethods">
			<xsl:call-template name="qualifiednodes"/>
		</xsl:variable>
		<xsl:if test="$qualifiedmethods > 0">
			...
			<xsl:if test="not ($isFunctionReport)">
				<tr>
					...
					<td class="{$cellClass} mtHdRightTop">
						<xsl:attribute name="colspan" xml:space="preserve">
							<xsl:choose>
								<xsl:when test="$showThreshold = 'True'">4</xsl:when>
			...
			<tr>
				<td>
					<xsl:attribute xml:space="preserve" name="class">
						<xsl:value-of select="$cellClass"/>
						<xsl:choose>
							<xsl:when test="$isFunctionReport">mtHdLeftFunc</xsl:when>
							<xsl:otherwise>mtHdLeft</xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
			...
			<tr>
				<xsl:choose>
					<xsl:when test="$showThreshold='True'">
						<td>
							<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>
								mtcItem
							</xsl:attribute>
							<xsl:if test="$styleTweak != ''" xml:space="preserve">
						...
						<td>
							<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mR
							</xsl:attribute>
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mPt
					</xsl:attribute>
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>tmcGraph
					</xsl:attribute>
	...
	<xsl:template name="cyclomaticComplexitySummaryCommon">
		...
		<xsl:for-each select="$root/mod[string(stats/@ex) != '1']">
			...
			<xsl:variable name="classcount">
				<xsl:choose xml:space="preserve">
					<xsl:when test="$reportType = 'SymbolCCModuleClassFailedCoverageTop'">
						<xsl:value-of select="count(ns/class[string(stats/@ex) != '1' and stats/@ccmax > 0 and
                                        (
					...
					<xsl:when test="$reportType = 'MethodCCModuleClassCoverageTop'">
						<xsl:value-of select="count(ns/class[string(stats/@ex) != '1' and stats/@ccmax > 0])"/>
					</xsl:when>
			...
			<xsl:if test="$classcount > 0">
				...
				<tr>
					<td class="secondaryChildTable ctHeader" colspan="{$tablecolumns}">
						Classes
						<xsl:if test="$printMethods = 'True'">/ Methods</xsl:if>
					</td>
				</tr>
				<xsl:choose>
					<xsl:when test="$reportType = 'SymbolCCModuleClassFailedCoverageTop'">
		...
		<xsl:for-each select="$root/mod[string(stats/@ex) != '1']">
			...
			<xsl:if test="$qualifiedmethods > 0">
				...
				<tr>
					<td class="secondaryChildTable ctHeader" colspan="{$tablecolumns}">
						Namespace
						<xsl:if test="$printClasses = 'True'">/ Classes</xsl:if>
					</td>
				</tr>
	...
	<xsl:template name="diffCoverageDetail">
		...
		<xsl:if test="$qualifiedmethods > 0">
			<tr>
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mtcItem
					</xsl:attribute>
					<xsl:if test="$styleTweak != ''" xml:space="preserve">
						<xsl:attribute name="style">
							<xsl:value-of select="$styleTweak"/>
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mR1
						<xsl:choose>
							<xsl:when test="$coveragesymbol - $coveragesymbol-diff &lt; 0">dn</xsl:when>
							<xsl:when test="$coveragesymbol - $coveragesymbol-diff &gt; 0">up</xsl:when>
							<xsl:otherwise></xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
					<xsl:call-template name="printcoverage">
				...
				<xsl:if test="$reportBranchPoints">
					...
					<td>
						<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mR1
							<xsl:choose>
								<xsl:when test="$coveragebranch - $coveragebranch-diff &lt; 0">dn</xsl:when>
								<xsl:when test="$coveragebranch - $coveragebranch-diff &gt; 0">up</xsl:when>
								<xsl:otherwise></xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
						<xsl:call-template name="printcoverage">
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mR1
						<xsl:choose>
							<xsl:when test="$coverageline - $coverageline-diff &lt; 0">dn</xsl:when>
							<xsl:when test="$coverageline - $coverageline-diff &gt; 0">up</xsl:when>
							<xsl:otherwise></xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
					<xsl:call-template name="printcoverage">
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mR1
						<xsl:choose>
							<xsl:when test="$coveragemethod - $coveragemethod-diff &lt; 0">dn</xsl:when>
							<xsl:when test="$coveragemethod - $coveragemethod-diff &gt; 0">up</xsl:when>
							<xsl:otherwise></xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
					<xsl:call-template name="printcoverage">
				...
				<xsl:if test="$reportCC">
					<td>
						...
						<xsl:choose>
							<xsl:when test="$stats/@diffdata = 1">
								<xsl:value-of select="format-number($stats/@dccavg,'#0.0')"/>
								/
								<xsl:value-of select="$stats/@dccmax"/>
							</xsl:when>
							<xsl:otherwise>
								-
							</xsl:otherwise>
					...
					<td>
						...
						<xsl:value-of select="format-number($stats/@ccavg,'#0.0')"/>
						/
						<xsl:value-of select="$stats/@ccmax"/>
					</td>
					...
					<td>
						<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mR
							<xsl:choose>
								<xsl:when test="$stats/@ccmax - $stats/@dccmax &lt; 0">dn</xsl:when>
								<xsl:when test="$stats/@ccmax - $stats/@dccmax &gt; 0">up</xsl:when>
								<xsl:otherwise></xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
						<xsl:choose>
							<xsl:when test="$stats/@diffdata = 1">
								<xsl:value-of select="format-number($stats/@ccavg - $stats/@dccavg,'#0.0')"/>
								/
								<xsl:value-of select="$stats/@ccmax - $stats/@dccmax"/>
							</xsl:when>
							<xsl:otherwise>
								-
							</xsl:otherwise>
				...
				<xsl:if test="$qualifiedmethods > 0">
					...
					<tr>
						<td class="secondaryChildTable ctHeader" valign="bottom">
							<xsl:attribute name="colspan" xml:space="preserve">
								<xsl:choose>
									<xsl:when test="$threshold = 'True'">
										<xsl:value-of select="$tablecolumns"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="$tablecolumns"/>
									</xsl:otherwise>
	...
	<xsl:template name="trendTDAttributes">
		...
		<xsl:attribute name="colspan" xml:space="preserve">
			<xsl:choose>
				<xsl:when test="$threshold = 'True'">6</xsl:when>
				<xsl:otherwise>5</xsl:otherwise>
			</xsl:choose>
		</xsl:attribute>
		<xsl:attribute xml:space="preserve" name="data">
			<xsl:for-each select="$stats/tp">
				<xsl:value-of select="format-number(@v,'#0')"/>
				<xsl:if test="position() != last()">,</xsl:if>
			</xsl:for-each>
		</xsl:attribute>
		<xsl:attribute xml:space="preserve" name="min">
		...
		<xsl:if test="$qualifiedmethods > 0">
			...
			<tr>
				...
				<td class="{$cellClass} mtHdRightTop">
					<xsl:attribute name="colspan" xml:space="preserve">
						<xsl:choose>
							<xsl:when test="$showThreshold = 'True'">5</xsl:when>
							<xsl:otherwise>4</xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
					<xsl:choose>
						<xsl:when test="$trendCoverageMetric = 'SequencePoints' or $trendCoverageMetric = 'SymbolCoverage'">Symbol
							Coverage
						</xsl:when>
						<xsl:when test="$trendCoverageMetric = 'BranchPoints' or $trendCoverageMetric = 'BranchCoverage'">Branch
							Coverage
						</xsl:when>
	...
	<xsl:template name="trendsCoverageDetail">
		...
		<xsl:variable name="total">
			<xsl:choose xml:space="preserve">
				<xsl:when test="$trendCoverageMetric = 'SequencePoints' or $trendCoverageMetric = 'SymbolCoverage'">
					<xsl:value-of select="$stats/@usp + $stats/@vsp"/>
				</xsl:when>
				<xsl:when test="$trendCoverageMetric = 'BranchPoints' or $trendCoverageMetric = 'BranchCoverage'">
					<xsl:value-of select="$stats/@ubp + $stats/@vbp"/>
				</xsl:when>
				<xsl:when test="$trendCoverageMetric = 'MethodCoverage'">
					<xsl:value-of select="$stats/@um + $stats/@vm"/>
				</xsl:when>
				<xsl:when test="$trendCoverageMetric = 'LineCoverage'">
					<xsl:value-of select="$stats/@ul + $stats/@vl"/>
				</xsl:when>
				<xsl:when test="$trendCoverageMetric = 'CyclomaticComplexity'">
					<xsl:value-of select="$stats/@ccmax"/>
				</xsl:when>
				<xsl:otherwise>0</xsl:otherwise>
			</xsl:choose>
		...
		<xsl:variable name="coverage">
			<xsl:choose xml:space="preserve">
				...
				<xsl:when test="$trendCoverageMetric = 'BranchPoints' or $trendCoverageMetric = 'BranchCoverage'">
					<xsl:value-of select="($stats/@vbp div $total) * 100"/>
				</xsl:when>
				<xsl:when test="$trendCoverageMetric = 'MethodCoverage'">
					<xsl:value-of select="($stats/@vm div $total) * 100"/>
				</xsl:when>
				<xsl:when test="$trendCoverageMetric = 'LineCoverage'">
					<xsl:value-of select="($stats/@vl div $total) * 100"/>
				</xsl:when>
		...
		<xsl:variable name="coverageclass" xml:space="preserve">
			<xsl:choose>
				<xsl:when test="$coverage = '-' or threshold = '-'"></xsl:when>
				<xsl:when test="$coverage &gt;= $threshold">up</xsl:when>
				<xsl:when test="$coverage &lt; $threshold">dn</xsl:when>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="min" xml:space="preserve">
			<xsl:call-template name="min">
				<xsl:with-param name="pSeq" select="$stats/tp[@v > -1]/@v"></xsl:with-param>
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="max" xml:space="preserve">
			<xsl:call-template name="max">
				<xsl:with-param name="pSeq" select="$stats/tp/@v"></xsl:with-param>
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="firsttrend" xml:space="preserve">
			<xsl:choose>
				<xsl:when test="$stats/tp[position() = 1]/@v = -1">0</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$stats/tp[position() = 1]/@v"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="lasttrend" xml:space="preserve">
			<xsl:choose>
				<xsl:when test="$stats/tp[position() = last()]/@v = -1">0</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$stats/tp[position() = last()]/@v"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="fulltrendamount" xml:space="preserve">
			<xsl:value-of select="$lasttrend - $firsttrend"/>
		</xsl:variable>
		<xsl:variable name="fulltrendclass" xml:space="preserve">
			<xsl:choose>
				<xsl:when test="$fulltrendamount &gt; 0">up</xsl:when>
				<xsl:when test="$fulltrendamount &lt; 0">dn</xsl:when>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:if test="$qualifiedmethods > 0">
			<tr>
				<xsl:choose>
					<xsl:when test="$showThreshold='True'">
						<td>
							...
							<xsl:attribute name="style">padding-right:0px;
								<xsl:value-of select="$styleTweak"/>
							</xsl:attribute>
							<xsl:choose>
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class">
						<xsl:if test="$showBottomLine = 'True'">mb</xsl:if>
						mR
						<xsl:value-of select="$coverageclass"/>
					</xsl:attribute>
					<xsl:choose>
				...
				<td>
					<xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb</xsl:if>mPt mR
						<xsl:value-of select="$fulltrendclass"/>
					</xsl:attribute>
	...
	<xsl:template name="qualifiednodes" xml:space="preserve">
		<xsl:choose>
			...
			<xsl:when test="$isDiffReport">
				<xsl:choose>
					...
					<xsl:when test="$diffAtLeast &lt; 0">
						<xsl:value-of select="count(.//stats[
                                                            @diffexists='1'
                                                            and
                                                            (
					...
					<xsl:otherwise>
						<xsl:value-of select="count(.//stats[@diffexists='1'])"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="$isTrendReport">1</xsl:when>
			<xsl:when test="string($atleastpercenttype) = 'branch'">
				<xsl:choose>
					<xsl:when test="name(.) = 'method'">
						<xsl:value-of
								select="count(stats[(@vbp div (@ubp + @vbp)) >= $atleastpercent and (@vbp div (@ubp + @vbp)) &lt;= $atmostpercent])"/>
					</xsl:when>
					<!--if rdf is used we have no way to show the data.-->
					<xsl:when test="count(.//method) = 0">1</xsl:when>
					<xsl:otherwise>
						<xsl:value-of
								select="count(.//method/stats[(@vbp div (@ubp + @vbp)) >= $atleastpercent and (@vbp div (@ubp + @vbp)) &lt;= $atmostpercent])"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="name(.) = 'method'">
						<xsl:value-of
								select="count(stats[(@vsp div (@usp + @vsp)) >= $atleastpercent and (@vsp div (@usp + @vsp)) &lt;= $atmostpercent])"/>
					</xsl:when>
					<!--if rdf is used we have no way to show the data.-->
					<xsl:when test="count(.//method) = 0">1</xsl:when>
					<xsl:otherwise>
						<xsl:value-of
								select="count(.//method/stats[(@vsp div (@usp + @vsp)) >= $atleastpercent and (@vsp div (@usp + @vsp)) &lt;= $atmostpercent])"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template name="style" xml:space="preserve">
		<style>
			<xsl:text disable-output-escaping="yes">
				...
				.mtgHd { border-bottom: 1px solid; border-top: 1px solid; border-right: 1px solid; text-align: left;
				font-weight: bold; }
				.mtgHd2 { border-bottom: 1px solid; border-top: 1px solid; border-right: 1px solid; text-align: center;
				font-weight: bold; }
				.mtDiv {width:500px;overflow: hidden;}

				.mtcItem { background: #ffffff; border-left: #dcdcdc 1px solid; border-right: #dcdcdc 1px solid; padding-left:
				10px; padding-right: 10px; font-weight: bold; font-size: 10px; }
				...
				.ctHeader { border-top: 1px solid; border-bottom: 1px solid; border-left: 1px solid; border-right: 1px solid;
				font-weight: bold; padding-left: 10px; }
				.ctciItem { background: #ffffff; border-left: #dcdcdc 1px solid; border-right: #dcdcdc 1px solid; padding-right:
				10px; font-size: 10px; }
				.etcItem { background: #ffffff; border-left: #dcdcdc 1px solid; border-right: #dcdcdc 1px solid; padding-left:
