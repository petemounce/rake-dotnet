...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="Message">
		...
		<xsl:choose>
			<xsl:when test="../../self::Resource">
				<xsl:apply-templates select="." mode="moduleSuppression"/>
			</xsl:when>
			<xsl:when test="../../self::Namespace">
				<xsl:apply-templates select="." mode="moduleSuppression"/>
			</xsl:when>
			<xsl:otherwise>[<xsl:if test="../../self::Module">module:</xsl:if>SuppressMessage("<xsl:value-of
					select="@Category"/>", "<xsl:value-of select="@CheckId"/>:<xsl:value-of select="@TypeName"/>")]
			</xsl:otherwise>
		</xsl:choose>
		<xsl:choose>
			<xsl:when test="../../self::Resource"></xsl:when>
	...
	<xsl:template match="Issue">
		...
		<xsl:if test="@Path">
			Source Code:<xsl:value-of select="@Path"/>\<xsl:value-of select="@File"/>(<xsl:value-of select="@Line"/>)
		</xsl:if>
		Resolution:
	...
	<xsl:template match="Type" mode="parent">
		<xsl:if test="not(../../@Name='')"><xsl:apply-templates select=".." mode="parent"/>.
		</xsl:if>
		<xsl:value-of select="@Name"/>
	</xsl:template>
	...
	<xsl:template match="Module" mode="module">[<xsl:value-of select="@Name"/>]
	</xsl:template>
	...
	<xsl:template match="Message" mode="moduleSuppression">[module: SuppressMessage("<xsl:value-of select="@Category"/>",
		...
		<xsl:if test="ancestor::Resource">
			<xsl:if test="@Id">, MessageId="<xsl:value-of select="@Id"/>"
			</xsl:if>
		</xsl:if>
		)]
	</xsl:template>
