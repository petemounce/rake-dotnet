...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/FxCopReport">
		<html>
			...
			<script>
				...
				if (block != null)
				{
				if (block.className == 'NodeDiv' || block.className == 'MessageBlockDiv' || IsMessageDivWithActionNone(block,
				how))
				{
			...
			<body bgcolor="white" alink="Black" vlink="Black" link="Black">
				...
				<table>
					<tr>
						<td class="Button" Id="ExpandAllButton" OnMouseOver="ButtonState('ExpandAllButton');"
						    OnMouseOut="ButtonState('ExpandAllButton');">
							<a href="javascript:ExpandAll()">
								<xsl:value-of select="/FxCopReport/Localized/String[@Key='ExpandAll']/text()"/>
							</a>
						</td>
						<td class="Button" Id="CollapseAllButton" OnMouseOver="ButtonState('CollapseAllButton');"
						    OnMouseOut="ButtonState('CollapseAllButton');">
							<a href="javascript:CollapseAll();">
								<xsl:value-of select="/FxCopReport/Localized/String[@Key='CollapseAll']/text()"/>
							</a>
						</td>
					</tr>
				</table>
				...
				<xsl:choose>
					<xsl:when test="$MessageCount > 0">
						...
						<div class="NodeLine">
							...
							<nobr class="MessageNumber">,<xsl:value-of select="$MessageErrorCount"/>&#32;<xsl:value-of
									select="/FxCopReport/Localized/String[@Key='Errors']/text()"/>,<xsl:value-of
									select="$MessageWarningCount"/>&#32;<xsl:value-of
									select="/FxCopReport/Localized/String[@Key='Warnings']/text()"/>.
							</nobr>
						</div>
	...
	<xsl:template match="Messages">
		...
		<div class="MessageBlock">
			...
			<xsl:value-of select="/FxCopReport/Localized/String[@Key='Messages']/text()"/>
		</div>
			...
			<table width="100%">
				<tr>
					...
					<td class="ColumnHeader">
						<xsl:value-of select="/FxCopReport/Localized/String[@Key='Status']/text()"/>
					</td>
					<td class="ColumnHeader">
						<xsl:value-of select="/FxCopReport/Localized/String[@Key='Certainty']/text()"/>
					</td>
					<td class="ColumnHeader" width="100%">
						<xsl:value-of select="/FxCopReport/Localized/String[@Key='Resolution']/text()"/>
					</td>
				</tr>
	...
	<xsl:template match="Issue">
		...
		<tr>
			...
			<td valign="top">
				<a href="javascript:DoNothing();">
					<nobr class="NodeIcon">
						<xsl:attribute name="id"><xsl:value-of select="$messageId"/>Toggle
						</xsl:attribute>
						&#x0034;
					</nobr>
				</a>
		...
		<xsl:if test="@Path">
			<tr class="SourceCode">
				...
				<td class="PropertyName">
					<xsl:value-of select="/FxCopReport/Localized/String[@Key='Source']/text()"/>
				</td>
				<td class="PropertyContent" colspan="2">
					...
					(
					<xsl:value-of select="/FxCopReport/Localized/String[@Key='Line']/text()"/>
	...
	<xsl:template match="Description" mode="ruledetails">
		<tr>
			<td class="PropertyName"><xsl:value-of select="/FxCopReport/Localized/String[@Key='RuleDescription']/text()"/>:
			</td>
	...
	<xsl:template match="File" mode="ruledetails">
		<tr>
			<td class="PropertyName"><xsl:value-of select="/FxCopReport/Localized/String[@Key='RuleFile']/text()"/>:
			</td>
			<td class="PropertyContent">
				<xsl:value-of select="@Name"/>
	...
	<xsl:template match="Url" mode="ruledetails">
		<tr>
			<td class="PropertyName"><xsl:value-of select="/FxCopReport/Localized/String[@Key='Help']/text()"/>:
			</td>
			<td class="PropertyContent">
				<a>
					<xsl:attribute name="href">
	...
	<xsl:template match="Rule" mode="ruledetails">
		<tr>
			<td class="PropertyName"><xsl:value-of select="/FxCopReport/Localized/String[@Key='Rule']/text()"/>:
			</td>
			<td class="PropertyContent">
				<xsl:value-of select="Name"/>
			</td>
		...
		<tr>
			<td class="PropertyName"><xsl:value-of select="/FxCopReport/Localized/String[@Key='Category']/text()"/>:
			</td>
			<td class="PropertyContent">
		...
		<tr>
			<td class="PropertyName"><xsl:value-of select="/FxCopReport/Localized/String[@Key='CheckId']/text()"/>:
			</td>
			<td class="PropertyContent">
