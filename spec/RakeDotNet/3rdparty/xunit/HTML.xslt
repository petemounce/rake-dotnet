...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="assembly">
		<xsl:text disable-output-escaping="yes">
			<![CDATA[<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">]]></xsl:text>
		<html>
			<head>
				<title>xUnit.net Test Results -
				...
				<style type="text/css">
					...
					.success, .failure, .skipped { font-family: Arial Unicode MS; font-weight: normal; float: left; width: 1em;
					display: block; }
					.success { color: #0c0; }
					.failure { color: #c00; }
					.skipped { color: #cc0; }
			...
			<body>
				...
				<div>
					...
					,
					Run time:
					<b><xsl:value-of select="@time"/>s
					</b>
				</div>
				<xsl:if test="//test[@result='Fail']">
					<br/>
					<h2><a name="failures"></a>Failed tests
					</h2>
				...
				<xsl:if test="//class/failure">
					<br/>
					<h2><a name="failures"></a>Failed fixtures
					</h2>
				...
				<xsl:if test="@skipped > 0">
					<br/>
					<h2><a name="skipped"></a>Skipped tests
					</h2>
	...
	<xsl:template match="test">
		<div>
			<xsl:attribute name="class"><xsl:if test="(position() mod 2 = 0)">alt</xsl:if>row
			</xsl:attribute>
			<xsl:if test="@result!='Skip'">
				<span class="timing"><xsl:value-of select="@time"/>s
				</span>
			</xsl:if>
			<xsl:if test="@result='Skip'">
	...
	<xsl:template match="class">
		<h3>
			...
			<span class="clickable">
				<xsl:attribute name="onclick">ToggleClass('class<xsl:value-of select="position()"/>')
				</xsl:attribute>
				<xsl:attribute name="ondblclick">ToggleClass('class<xsl:value-of select="position()"/>')
				</xsl:attribute>
				...
				<xsl:value-of select="@name"/>
				&#160;
				<span class="testcount">(<xsl:value-of select="@total"/>&#160;test<xsl:if test="@total > 1">s</xsl:if>)
				</span>
			</span>
			<br clear="all"/>
		</h3>
