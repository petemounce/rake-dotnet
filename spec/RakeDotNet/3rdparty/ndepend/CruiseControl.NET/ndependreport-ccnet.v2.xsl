...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="NDepend">
		...
		<div class="biginfo">
			To get explanations concerning these metrics, go to the
			<a HREF="http://www.NDepend.com" target="_blank">NDepend home page.</a>
		</div>
		<p></p>
		<p></p>
		<A HREF="#APPMETRIC">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<div class="info">At least one cycle exists in the assemblies dependencies. The build order cannot be
					computed.
				</div>
			</xsl:otherwise>
		</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="@Status='Warn'">
						<div class="cql_warning">
							...
							on
							<xsl:value-of select="@NbNodeTested"/>
							tested match the condition. --> Group {<xsl:value-of select="$groupname"/>}
						</div>
					</xsl:when>
					<xsl:otherwise>
						<div class="cql_error">
							ERROR: The following CQL query doesn't compile. Description:
